...
6. **Phase 3.1 — Hook classification.** Read first line / first sentence. Map to 14-category taxonomy in `references/hook-taxonomy.md`. Output: count, %, avg engagement per hook type.
7. **Phase 3.2 — Post type (pillars).** Map to Educational / Personal / Promotional / Organizational / Engagement. Compare mix to 40/25/25/10 target. Definitions → `references/classification-dimensions.md`.
...
10. **Phase 3.5 — CTA classification.** Analyze final 1-3 lines. Bucket into 8 types: comment prompt, DM invite, link/resource, follow/connect, save, repost, no CTA, multiple CTAs. Output: distribution + avg engagement per type. Pattern examples → `references/classification-dimensions.md`.
...
13. **Phase 4 — Cross-profile comparison** (only if 2+ profiles). Build profile overview matrix; format mix table; hook style heat map; engagement benchmarks; CTA distribution; top patterns to emulate (evidence-cited); anti-patterns to avoid (evidence-cited). Detail → `references/classification-dimensions.md`.
14. **Self-evaluation.** Completeness (all phases + profiles + comparison if applicable); accuracy (3 random hook spot-checks, engagement avg sanity, format consistency vs attachments); honesty (zero invented numbers, no guessed media content, gaps marked). Full protocol → `references/quality-and-evaluation.md`.
15. **Write outputs.** Per-profile: `linkedin-audit-{username}.md`. Cross-profile: `linkedin-audit-comparison.md`. Templates → `references/output-templates.md`.

## What good looks like

**References:**
- `references/process-flowchart.md` — 6-phase ASCII flow with checkpoints + review gate
- `references/hook-taxonomy.md` — 14 hook categories with definitions and examples
- `references/classification-dimensions.md` — Full 6-dimension taxonomy + Phase 4 cross-profile method
- `references/output-templates.md` — Per-profile + comparison output skeletons
- `references/quality-and-evaluation.md` — Anti-hallucination guardrails, pre-delivery checklist, self-eval protocol, integrations, MCP details, iteration prompts, triggers, full input spec, changelog
...
**Anti-hallucination (load-bearing):** Only analyze Apify-returned data. Never invent engagement counts, post text, or follower counts. Mark <10 posts as insufficient data. Source every finding `(Apify, harvestapi/linkedin-profile-posts, YYYY-MM-DD)`. When hook/CTA classification is ambiguous, mark as "ambiguous" and note the two closest matches. Use precise language ("23 of 25 posts (92%)" not "almost all"). Full guardrails → `references/quality-and-evaluation.md`.

## Push

Default destination: `client_folder/linkedin/` (Google Docs export). Output filenames: `linkedin-audit-{username}.md` per profile + `linkedin-audit-comparison.md` when 2+ profiles. After delivery, prompt user with iteration options (deeper dive on a creator, run `linkedin-algo-audit` on top performers, build content guide from patterns, generate hooks from winning patterns, expand window/depth). Iteration prompt list → `references/quality-and-evaluation.md`.
