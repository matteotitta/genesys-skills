...
- `references-banned-patterns.md` — non-negotiable craft guards (load every time)
- `references-scoring-rubric.md` — 5-dimension × 0-4 scorecard for self-review
- `references-post-render-checklist.md` — final ship gate
- `references-quality-library.md` — distilled craft principles (layout, typeset, polish, motion, distill, harden, accessibility)
...
1. **Layout** → `references-quality-library.md` § Layout. Confirm grid, balance, ≤7 visible decision points.
2. **Distill** → `references-quality-library.md` § Distill. Cut content that doesn't serve the focal action.
3. **Typeset** → `references-quality-library.md` § Typeset. ≤2 font weights, General Sans Bold for display, no overused-fonts.
4. **Polish** → `references-quality-library.md` § Polish. Refinement craft — corner-radius alignment, optical balance, focal hierarchy.
5. **Anti-pattern guard** → `references-banned-patterns.md`. Non-negotiable. No gradient text, no side-stripes ≥2px, no hero-metric template, no icon-tile-above-heading, etc.
6. **Harden** (web-mock variant only) → `references-quality-library.md` § Harden. Contrast, focus states, responsiveness.
7. **Self-score** → `references-scoring-rubric.md`. Score the artifact 0-4 across 5 dimensions; require ≥3/dim and zero P0/P1 findings.
8. **Final ship gate** → Run `/premortem --output` to surface failure modes, then `/design-reviewer` as the final ship-ready gate. If `/design-reviewer` is unavailable, run `references-post-render-checklist.md` end-to-end.

**Authority + shared library.** In the workspace, `.claude/rules/design-production.md` is the governing design doctrine — read it first. The four `references-*.md` here are self-contained copies (for cloud / portable runs) that mirror the canonical shared craft library at `.claude/skills/meta/catalog/design-reviewer/references/` (`anti-patterns.md`, `scoring-rubric.md`, `harden-checklist.md`, `accessibility-checklist.md`, `positive-controls.md`). Attribution for the impeccable-derived content is in `NOTICE.md`.

## Web-mock variant — chain rule

When the user requests a web mock (vs. LinkedIn/Substack/carousel/infographic):

1. Read the design-production rule § "Web rendering path" (auto-loads in workspace; cite from memory if running cloud-only).
...
2. Run `/design-reviewer` as the final ship-ready gate — the universal review hook for visual output. If unavailable, run `references-post-render-checklist.md` end-to-end and require the scoring-rubric total ≥14/20 with zero P0 findings.
